...
public class BigFileTest {
    ...
    public void merge() throws IOException {
        String chunkPath="d:/bigfile_test/chunk/";
        File merge=new File(chunkPath);
        File[] files = merge.listFiles();
        String mergeFile="d:/bigfile_test/123.mp4";
        File mergefile=new File(mergeFile);
        File originalFile = new File("d:\\1.mp4");
        if (mergefile.exists()) {
            mergefile.delete();
        }
        RandomAccessFile file_write=new RandomAccessFile(mergefile,"rw");
            ...
            byte [] b=new byte[1024];
            for(File x:files){
                RandomAccessFile file_read = new RandomAccessFile(x, "r");
                int len=-1;
                while((len = file_read.read(b)) != -1){
                    file_write.write(b,0,len);
                }
                file_read.close();
            }
            file_write.close();
    ...
    public void uploadChunk(){
        String chunkFolderPath = "D:\\bigfile_test\\chunk";
